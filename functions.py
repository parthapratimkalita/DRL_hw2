...
def MonteCarloLearnStateValueFunction(env,stateNumber,numberOfEpisodes,discountRate):
    ...
    mean_valueFunctionEstimate = np.zeros(stateNumber)
    ...
    for indexEpisode in range(numberOfEpisodes):
        ...
        for indexCurrentState in range(numberOfVisitedStates-1,-1,-1):
            ...
            if stateTmp not in visitedStatesInEpisode[0:indexCurrentState]:
                ...
                mean_valueFunctionEstimate[stateTmp]=sumReturnForEveryState[stateTmp]/numberVisitsForEveryState[stateTmp]

                """ if you want to see the sate values in each episode
                just try to execute the commented lines (since it is very time consuming I am not going to plot them all the time)
          
              """
        #if (indexEpisode +1) % 100 == 0:
            #file_name = "monty_carlo_episode_" + str(indexEpisode) + ".jpg"   
            #grid_print(mean_valueFunctionEstimate,reshapeDim=4,fileNameToSave=file_name) 
    ###########################################################################
